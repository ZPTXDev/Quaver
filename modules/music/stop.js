...
module.exports.description = "Stop the current track and clear the queue.";
module.exports.action = async function action (details) {
    ...
    details["message"].channel.createMessage({
        messageReference: {messageID: details["message"].id},
        embed: {
            description: "Stopped the current track and cleared the queue successfully.",
            color: 0xf39bff
        }
    });
    return true;
