...
export default {
	...
	async execute(interaction: QuaverInteraction<AutocompleteInteraction>): Promise<void> {
		...
		return interaction.respond(
			player.queue.tracks
				.map((track, index): ApplicationCommandOptionChoiceData & { title: string, requester: Snowflake } => ({ name: `${index + 1}. ${track.title}`, value: index + 1, title: track.title, requester: track.requester }))
