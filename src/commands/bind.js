...
export default {
	...
	async execute(interaction) {
		...
		if (!channel.permissionsFor(interaction.client.user.id).has(new PermissionsBitField([PermissionsBitField.Flags.ViewChannel, PermissionsBitField.Flags.SendMessages]))) return interaction.replyHandler.locale('CMD.BIND.RESPONSE.PERMISSIONS_INSUFFICIENT', { vars: [channel.id], type: 'error' });
		...
		return interaction.replyHandler.locale('CMD.BIND.RESPONSE.SUCCESS', { vars: [channel.id], type: 'success' });
	},
};
