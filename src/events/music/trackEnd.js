...
export default {
	...
	async execute(queue, track, reason) {
		...
		if (reason === 'LOAD_FAILED') {
			...
			await queue.player.handler.locale('MUSIC.PLAYER.TRACK_SKIPPED_ERROR', { args: [track.title.length >= 50 ? `${track.title.substring(0, 47)}...` : track.title, track.uri, reason], type: 'warning' });
