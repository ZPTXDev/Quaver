...
import { colors, defaultLocaleCode } from '#settings';
...
export function msToTimeString(msObject, simple) {
	if (simple) {
		if (msObject['d'] > 0) {
			return getLocaleString(defaultLocaleCode, 'MISC.MORE_THAN_A_DAY');
		}
 ...
 */
export function getLocaleString(localeCode, stringPath, ...vars) {
	if (!locales.get(localeCode)) return 'LOCALE_MISSING';
	let categoryProps = locales.get(localeCode);
	let localeString = get(categoryProps, stringPath);
	if (!localeString) {
		// This uses 'en' on purpose.
		// 'en' is the only locale with a guaranteed 100% completion rate.
		categoryProps = locales.get('en');
		localeString = get(categoryProps, stringPath);
	}
 ...
 */
export async function getGuildLocaleString(guildId, stringPath, ...vars) {
	return getLocaleString(await data.guild.get(guildId, 'settings.locale') ?? defaultLocaleCode, stringPath, ...vars);
}
 ...
 */
export function checkLocaleCompletion(localeCode) {
	if (!locales.get(localeCode)) return 'LOCALE_MISSING';
	...
	iterateObject(englishStrings);
	foreignStringCount = englishStringCount - missingStrings.length;
	// missing strings
	if (englishStringCount > foreignStringCount) {
		return { completion: foreignStringCount / englishStringCount * 100, missing: missingStrings };
	}
 ...
 */
export function buildMessageOptions(msgData, { type = 'neutral', components = null, files = null } = {}) {
...
export async function settingsPage(interaction, guildLocaleCode, option) {
	let current;
	let embeds = [];
				...
				new SelectMenuBuilder()
					...
					.addOptions(
						fs.readdirSync(getAbsoluteFileURL(import.meta.url, ['..', '..', 'locales']))
							.map(file => ({ label: `${languageName[file] ?? 'Unknown'} (${file})`, value: file, default: file === guildLocaleCode })),
					),
			);
			break;
		case 'format': {
