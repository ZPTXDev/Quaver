...
function checkLocaleCompletion(language) {
	...
	let stringCount = 0;
	const stringFiles = readdirSync(join(__dirname, '..', '..', 'locales', 'en')).filter(file => file.endsWith('.json'));
	let missingStrings = [];
	for (const file of stringFiles) {
		const category = file.split('.')[0];
		const strings = require(join(__dirname, '..', '..', 'locales', 'en', `${category}.json`));
		const foreignStrings = require(join(__dirname, '..', '..', 'locales', language, `${category}.json`));
