...
export default class PlayerHandler {
	...
	async send(msgData, { type = 'neutral', components = null, files = null } = {}) {
		...
		const sendData = messageDataBuilder(msgData, { type, components, files });
	...
	async locale(code, { args = [], type = 'neutral', components = null, files = null } = {}) {
		return this.send(await getGuildLocale(this.interaction.guildId, code, ...args), { type, components, files });
	}
}
