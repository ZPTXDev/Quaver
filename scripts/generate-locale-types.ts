...
// Type to represent a path with its value
type PathWithValue = {
    path: string;
    value: string;
};
 ...
 */
function generatePathsWithValues(
    obj: NestedObject,
    prefix: string = '',
): PathWithValue[] {
    const paths: PathWithValue[] = [];
 ...
 */
function escapeForJSDoc(str: string): string {
    return str
        .replace(/\\/g, '\\\\')
        .replace(/\*/g, '\\*')
        .replace(/\n/g, ' ')
 ...
 */
async function generateLocaleTypes(): Promise<void> {
    ...
    const allPathsWithValues: PathWithValue[] = [];
